...
class MtimeSchedule(BaseSchedule):
    ...
    def __init__(self, namespace=None, host="localhost", port=6379, db=0,
        ...
        try:
            ...
            self._prepare_to_process_queue_html = RedisQueue("%s:%s" % (self._namespace, "prepare-html",),
                                                        host=host, port=port, db=db)
            ...
            self.logger.error("init Mtime schedule failed error:%s" % e)
    ...
    def pop_task(self):
        ...
        try:
            ...
            self._count += 1
            if self._count % 3 == 0:
                if self._prepare_to_process_queue_js.size() > 0:
                    task = self._prepare_to_process_queue_js.pop()
                    return task
                if self._prepare_to_process_queue_html.size() > 0:
                    task = self._prepare_to_process_queue_html.pop()
                    return task
                else:
                    return None
            else:
    ...
    def clear_all(self):
        ...
        try:
            self._prepare_to_process_queue_html.clear()
            self._prepare_to_process_queue_js.clear()
