...
from tigerspider.spiders.ctrip.util import (HOTEL_SERVICE_CODES, ROOM_SERVICE_CODES,
    ...
    def parse(self, task, input_file):
        ...
        try:
            ...
            for elem in elems:
                ...
                if len(chinese_name) <= 0 or not city_code or len(city_code) <= 0 or len(ctrip_code) <= 0:
                    self.logger.info("invaliade city chinese_name:%s citycode:%s ctrip_code:%s"
            ...
            self.logger.info("city parser end to parse")
...
class HotelListParser(BaseParser):
    ...
    def parse(self, task, input_file):
        ...
        try:
            ...
            header = elems[0]
            if "ResultCode" not in header.attrib or header.attrib['ResultCode'] != "Success":
                ...
                for property_elem in property_elems:
                    hotel_code = str(property_elem.attrib['HotelCode']) if "HotelCode" in property_elem.attrib \
                        else None
                    hotel_ctrip_city_code = str(property_elem.attrib['HotelCityCode']) \
                        if "HotelCityCode" in property_elem.attrib else None
                    hotel_address = flist(property_elem.xpath(
                        "*[local-name()='Address']/*[local-name()='AddressLine']/text()"))
            ...
            self.logger.info("hotel parse end to parse")
    ...
    def parse(self, task, input_file):
        ...
        try:
            ...
            if "ResultCode" not in header.attrib or header.attrib['ResultCode'] != "Success":
                ...
                content_elems = xpath_namespace(tree, "/Response/HotelResponse/"
                ...
                for content_elem in content_elems:
                    ...
                    try:
                        ...
                        item_hotel_latitude = "" if position_elem is None or "Latitude" not in position_elem.attrib \
                            else position_elem.attrib.get('Latitude')
                        item_hotel_longitude = "" if position_elem is None or "Longitude" not in position_elem.attrib \
                        ...
                        item_hotel_service = u"、".join(
                            [flist(service.xpath("*[local-name()='DescriptiveText']/text()"))
                             ...
                             if "Code" in service.attrib and service.attrib["Code"] in HOTEL_SERVICE_CODES])

                        item_room_service = u"、".join(
                            [flist(service.xpath("*[local-name()='DescriptiveText']/text()"))
                             for service in service_elems
                             if "Code" in service.attrib and service.attrib["Code"] in ROOM_SERVICE_CODES])
                        ...
                        image_elems = [] if multimedia_elem is None else xpath_namespace(
                            multimedia_elem, "MultimediaDescription/ImageItems/ImageItem")
                        ...
                        for image_elem in image_elems:
                            ...
                            if item_hotel_code and image_url:
                                image_item = ImageItem(item_hotel_code, str(image_type),
                                                       unicode(image_text), str(image_url))
                                yield image_item
                        ...
                        for room_elem in room_elems:
                            ...
                            if room_info_id and room_info_name and item_hotel_code:
                                ...
                                room_item = RoomInfoItem(item_hotel_code, str(room_info_id), room_info_name, room_floor,
                                                         room_net_service, room_net_service_fee,
                                                         room_bed_type, room_breakfast, room_area)
                                yield room_item

                        item_hotel_address = "" if item_hotel_code not in hotel_address_dict \
                            else hotel_address_dict.get(item_hotel_code)
                        hotel_item = HotelInfoItem(item_hotel_code, item_hotel_city_code, item_hotel_name,
                                                   item_hotel_brand_id, item_hotel_latitude,
                                                   item_hotel_longitude, item_hotel_service,
                                                   item_room_service, item_hotel_star, item_hotel_rate,
                        ...
                        if item_hotel_code is None:
                            ...
                            chinese_name = task.kwargs.get('chinesename')
                            yield build_rooms_task_for_hotel([item_hotel_code], item_hotel_city_code,
